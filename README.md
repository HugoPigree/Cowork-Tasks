...
Optional: set `VITE_API_BASE` in `frontend/.env` if the API is on another origin (then ensure `CORS_ALLOWED_ORIGINS` in Django `.env` includes your dev URL, e.g. `http://localhost:5173`).
